    ...
    <div class="left-panel">
        ...
        <div class="form-card">
            @if($workSites->count() > 0)
                <form method="POST" action="{{ route('staff.daily-updates.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <input type="date" name="date" class="mobile-input" value="{{ date('Y-m-d') }}" required>
        ...
        @forelse($updates as $upd)
            <div class="update-card">
                <div class="update-top">
                    <div>
                        <div class="update-date">{{ $upd->date->format('d M Y') }}</div>
                        <div class="update-site">{{ $upd->workSite?->site_name ?? 'Unknown site' }}</div>
                    </div>
                    <div class="update-meta">
                        {{ $upd->created_at->diffForHumans() }}
                    </div>
                </div>

                @if($upd->photo)
                    <a href="{{ asset('storage/'.$upd->photo) }}" target="_blank">
                        <img src="{{ asset('storage/'.$upd->photo) }}" class="update-photo" alt="Site photo">
                    </a>
                @endif

                @if($upd->note)
                    <p class="update-note">{{ $upd->note }}</p>
                @else
...
<div class="bottom-nav">
    ...
    <a href="{{ route('staff.tickets') }}"><i>🎫</i>Tickets</a>
    <a href="{{ route('staff.daily-updates') }}" class="active"><i>📋</i>Updates</a>
    <a href="{{ route('expenses.my') }}"><i>🧾</i>Expenses</a>
</div>
