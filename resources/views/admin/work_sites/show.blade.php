...
<div class="site-hero d-flex justify-content-between align-items-start gap-3"><div><small>WORK SITE</small><h2 class="mb-1">{{ $workSite->site_name }}</h2><div>{{ $workSite->client_name }} · {{ $workSite->location }}</div></div><div class="d-flex gap-2 flex-wrap"><a class="btn btn-success btn-sm" href="{{ route('admin.work-sites.wages.index', $workSite) }}">💰 Wages</a><a class="btn btn-warning btn-sm" href="{{ route('admin.work-sites.inventory',$workSite) }}">Inventory</a><a class="btn btn-light btn-sm" href="{{ route('admin.work-sites.edit',$workSite) }}">Edit Site</a><a class="btn btn-outline-light btn-sm" href="{{ route('admin.work-sites') }}">Back</a></div></div>
